...
function highlightLine(line: string): React.ReactNode[] {
  ...
  const keywordSet = new Set([
    "function", "class", "import", "from", "export", "return", "if", "else",
    "while", "for", "in", "switch", "case", "break", "continue", "try",
    "catch", "throw", "const", "let", "var", "new", "extends", "implements",
    "interface", "type", "async", "await", "true", "false", "null", "undefined",
  ]);

  const parts = line.split(
    /(\b(?:function|class|import|from|export|return|if|else|while|for|in|switch|case|break|continue|try|catch|throw|const|let|var|new|extends|implements|interface|type|async|await|true|false|null|undefined|Promise|Record|Map|Set)\b|"(?:[^"\\]|\\.)*"|'(?:[^'\\]|\\.)*'|`(?:[^`\\]|\\.)*`|\/\/.*$|\b\d+(?:\.\d+)?\b)/
  );
  ...
  return parts.map((part, idx) => {
    ...
    if (["Promise", "Record", "Map", "Set"].includes(part)) {
