    ...
    if (m) {
      ...
      for (let j = i + 1; j < lines.length; j++) {
        if (
          ...
          (lines[j].match(/^\S/) && lines[j].trim() !== "" && !lines[j].startsWith("//") && !lines[j].startsWith("/*") && !lines[j].startsWith("*") && !lines[j].startsWith("import") && !lines[j].startsWith("export const") && !lines[j].startsWith("const"))
        ) {
          endLine = j;
          break;
        }
...
function extractFunctions(
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (m) {
      functions.push({
        name: m[1],
        signature: lines[i].trim().replace(/\{$/, "").trim(),
        startLine: i + 1,
      });
    }
  }
...
// Looks for name: "tool_name" patterns in object literals
